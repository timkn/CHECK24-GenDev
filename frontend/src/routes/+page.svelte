<script lang="ts">
  ...
  function handleDestinationDescriptionCard(
    destination: string,
    dateFrom: string,
    dateTo: string,
    countAdults: number,
  ...
  function handleSubmit() {
    ...
    if (
      counterChildren == null ||
      counterChildren < 0 ||
      counterChildren > 10 ||
      isNaN(counterChildren)
    ) {
      childsFail = true;
      open_all();
      return;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        handleDestinationDescriptionCard(
          paparamDestination,
          paramDateFrom,
          paramDateTo,
          paramCountAdults,
...
<div class="m-6">
  <p
    class="m-2 text-lg font-normal text-gray-500 text-center lg:text-xl dark:text-gray-400"
  >
    {getWelcomeText()}
  </p>
  <h1
    class="m-2 text-4xl font-extrabold leading-none tracking-tight text-gray-00 text-center md:text-xl lg:text-3xl dark:text-white"
  >
...
<Accordion multiple class="m-8">
    ...
    <form>
      <Alert color="dark" class="px-3 py-2 flex-wrap">
        <svelte:fragment slot="icon">
          <Button on:click={() => (explainationAIModal = true)}
            >Erklärung</Button
          >
          <Modal
            id="default-modal"
            title="Terms of Service"
            bind:open={explainationAIModal}
            autoclose
          >
            <p
              class="text-base leading-relaxed text-gray-500 dark:text-gray-400"
            >
              Mithilfe von Künstlicher Intelligenz werden Ihen Urlaubsorte
              vorgeschlagen. Probieren Sie zum Beispiel "Ich möchte in eine eine
              echte Großstadt, aber auch ans Meer" oder "Ich möchte auf eine
              Insel" oder "Ich möchte in eine historische Weltmetropole".
            </p>
            <p
              class="text-base leading-relaxed text-gray-500 dark:text-gray-400"
            >
            ...
            <svelte:fragment slot="footer">
              <Button on:click={() => console.log('Handle "success"')}
                >Okay</Button
              >
            </svelte:fragment>
          </Modal>
          <Textarea
            bind:value={searchDestinationText}
            on:input={() => updateBadges(searchDestinationText)}
            id="chat"
            class="mx-4"
            rows="1"
  ...
  <AccordionItem bind:open={fromCards[2]}>
    <span slot="header" class="flex flex-row gap-4 flex-wrap">
      ...
      <Badge large
        ><p>{toGermanDate(dateFrom)} → {toGermanDate(dateTo)}</p></Badge
      >
    </span>
  ...
  <AccordionItem bind:open={fromCards[3]}>
    <span slot="header" class="flex flex-row gap-4 flex-wrap">
      ...
      <Badge large>Erwachsene: {counterAdults}</Badge>
      <Badge large>Kinder: {counterChildren}</Badge>
    </span>
    <div class="flex flex-row justify-center items-baseline gap-4 flex-wrap">
      ...
      <ButtonGroup>
        <Button
          outline
          color="red"
          on:click={() => (counterAdults > 0 ? counterAdults-- : counterAdults)}
          >-
        </Button>
        <Input
          type="number"
          id="first_name"
          bind:value={counterAdults}
          required
        />
        <Button
          outline
          color="green"
          on:click={() =>
            counterAdults < 20 ? counterAdults++ : counterAdults}
          >+
        </Button>
      </ButtonGroup>
      <p>Kinder</p>
      <ButtonGroup>
        <Button
          ...
          color="red"
          on:click={() =>
            counterChildren > 0 ? counterChildren-- : counterChildren}
          >-
        </Button>
        <Input
          type="number"
          id="first_name"
          bind:value={counterChildren}
          required
        />
        <Button
          outline
          color="green"
          on:click={() =>
            counterChildren < 20 ? counterChildren++ : counterChildren}
          >+
        </Button>
      </ButtonGroup>
    </div>
...
<ul class="grid grid-cols-2 gap-4 m-8">
  {#each offers.slice(0, limit) as offer, index}
    {#if offers.length > 3 ? index === 2 : index === 0}
      <div
        class="p-6 bg-white border border-gray-200 rounded-lg shadow col-span-2 justify-self-stretch"
      >
        <h5
        ...
        {:else}
          <div class="flex justify-center gap-4 items-center">
            ...
            >
              Beschreibung für Mallorca wird geladen...
            </p>
          </div>
        {/if}
      </div>
    {/if}
    <OfferCardHotel {userData} data={offer} />
  {/each}
</ul>
...
{#if offers.length === 0 && !loadingOffers && !isInitailOffers}
  <Alert class="m-8" color="red">
    <span class="font-medium">Keine Angebote gefunden.</span> Bitte wähle andere
    Reisedaten.
  </Alert>
{/if}
...
{#if noAirport}
  <Alert class="m-8" color="red">
    <span class="font-medium">Kein Flughafen ausgewählt.</span> Bitte wähle einen
    Flughafen aus.
  </Alert>
{/if}
...
{#if childsFail}
  <Alert class="m-8" color="red">
    <span class="font-medium"
      >Bitte gib einen gültigen Wert für Anzahl Kinder ein.</span
    > Bitte gebe eine Zahl ein / max 20.
  </Alert>
{/if}
...
{#if adultsFail}
  <Alert class="m-8" color="red">
    <span class="font-medium"
      >Bitte gib einen gültigen Wert für die Anzahl der Erwachsenen ein.</span
    > Bitte gebe eine Zahl ein / max 20.
  </Alert>
{/if}

<Alert class="m-8" color="red">
  <span class="font-medium">Die Suche ist zur Zeit nicht verfügbar.</span> Search
